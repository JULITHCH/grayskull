...
export class LlmClient {
  ...
  /** live estimate of tokens generated by the in-flight request (0 when idle);
   *  folded into cumCompletionTokens when the request finishes. */
  liveCompletionTokens = 0;
  /** measured ratio of real prompt tokens (vLLM usage) to the chars/4 estimate.
   *  chars/4 runs badly low on dense code (~1.4-1.5×), which once let a session
   *  sail past the compaction threshold straight into a context-overflow 400.
   *  Learned per request, floored at 1 — calibrates both the compaction check
   *  and the output clamp. */
  promptEstRatio = 1;
  /** raw prompt estimate of the in-flight request, kept for calibration. */
  private lastRawEst = 0;
  /** tool-schema token estimate of the latest request — schemas are part of
   *  the prompt but invisible to message-only estimates. */
  private lastEstTools = 0;

  /** Calibrated prompt-token estimate for a would-be request with these
   *  messages: chars/4 + tool schemas, scaled by the measured drift. This is
   *  what compaction checks must use — the raw estimate runs ~1.4× low on
   *  dense code and once let a session 400 on context overflow. */
  estimatePrompt(messages: ChatMessage[]): number {
    return Math.ceil((estimateMessagesTokens(messages) + this.lastEstTools) * this.promptEstRatio);
  }
  ...
  /** Total completion tokens this session, including the in-flight estimate —
   *  feeds the statusline token counter. */
  sessionTokens(): number {
    return this.cumCompletionTokens + this.liveCompletionTokens;
  }
  ...
  async complete(
    ...
    const rawEst = estimateMessagesTokens(messages) + estTools;
    this.lastRawEst = rawEst;
    this.lastEstTools = estTools;
    // fudge: at least the historic 20%, or the measured drift + 10% margin
    const fudge = Math.max(1.2, this.promptEstRatio * 1.1);
    const estPrompt = Math.ceil(rawEst * fudge) + 384;
  ...
  private async streamOnce(
    ...
    const bumpGen = (s: string): void => {
      ...
      this.liveCompletionTokens = estGen;
    ...
    try {
      ...
      // the tokens were still generated — keep the estimate in the session total
      this.cumCompletionTokens += estGen;
      this.liveCompletionTokens = 0;
    ...
    if (usage) {
      ...
      // calibrate the estimator against the authoritative count (recent-weighted,
      // floored at 1 so prose-heavy sessions never scale estimates down)
      if (this.lastRawEst > 0 && usage.promptTokens > 0) {
        const observed = usage.promptTokens / this.lastRawEst;
        this.promptEstRatio = Math.min(3, Math.max(1, this.promptEstRatio * 0.3 + observed * 0.7));
      }
  ...
  async oneShot(system: string, user: string, maxTokens = 4096): Promise<string> {
    ...
    if (res.usage) this.cumCompletionTokens += res.usage.completion_tokens;
